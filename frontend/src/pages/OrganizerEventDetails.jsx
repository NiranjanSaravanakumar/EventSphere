...
const OrganizerEventDetails = () => {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Loader2 style={{ width: '2rem', height: '2rem', color: 'var(--color-text-primary)', animation: 'spin 1s linear infinite' }} />
      </div>
    );
  }
  ...
  if (!event) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg-primary)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--color-text-primary)' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 600 }}>Event not found</h2>
        <button onClick={() => navigate(-1)} style={{ marginTop: '1rem', padding: '0.5rem 1rem', borderRadius: '8px', background: 'rgba(var(--glass-rgb),0.1)', color: 'var(--color-text-primary)', border: 'none', cursor: 'pointer' }}>Go Back</button>
      </div>
    );
  }
  ...
  const registrationPieOption = {
    tooltip: { trigger: 'item', backgroundColor: 'rgba(0,0,0,0.8)', borderColor: 'rgba(var(--glass-rgb),0.1)', textStyle: { color: 'var(--color-text-primary)' } },
    legend: { top: 'bottom', textStyle: { color: 'var(--color-text-primary)' } },
    series: [
      {
        name: 'Registration',
        type: 'pie',
        ...
        itemStyle: { borderRadius: 10, borderColor: 'var(--color-bg-primary)', borderWidth: 2 },
        label: { show: true, position: 'inside', formatter: '{c}', color: 'var(--color-text-primary)', fontWeight: 'bold' },
        emphasis: { label: { show: true, fontSize: '16', fontWeight: 'bold', color: 'var(--color-text-primary)' } },
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3rem' }}>
        <motion.button
          ...
          style={{
            ...
            borderRadius: '999px', background: 'rgba(var(--glass-rgb),0.05)', border: '1px solid rgba(var(--glass-rgb),0.1)',
        ...
        <motion.button
          ...
          style={{
            ...
            borderRadius: '999px', background: 'var(--color-text-primary)', border: 'none',
            color: 'var(--color-bg-primary)', fontSize: '0.875rem', fontWeight: 600, cursor: 'pointer'
          }}
        >
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 350px', gap: '2rem' }}>
        ...
        <div>
          <motion.div
            ...
            style={{
              padding: '2rem', borderRadius: '1.5rem', background: 'rgba(var(--glass-rgb),0.03)',
              border: '1px solid rgba(var(--glass-rgb),0.08)', marginBottom: '2rem'
            }}
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#D4D4D8' }}>
                <Calendar style={{ color: 'var(--color-text-subtle)' }} /> <span>{fmtDate(event.date)}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#D4D4D8' }}>
                <MapPin style={{ color: 'var(--color-text-subtle)' }} /> <span>{event.location}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#D4D4D8' }}>
                <Users style={{ color: 'var(--color-text-subtle)' }} /> <span>Capacity: {event.capacity}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#D4D4D8' }}>
          ...
          >
            ...
            {guests.length === 0 ? (
              <p style={{ color: 'var(--color-text-subtle)' }}>No guests registered yet.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {guests.map((g, i) => (
                  <div key={i} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '1rem', borderRadius: '1rem', background: 'rgba(var(--glass-rgb),0.02)',
                    border: '1px solid rgba(var(--glass-rgb),0.05)'
                  }}>
                    <div>
                      <div style={{ fontWeight: 500, color: 'var(--color-text-primary)' }}>{g.attendeeName}</div>
                      <div style={{ fontSize: '0.8125rem', color: 'var(--color-text-subtle)', marginTop: '0.25rem' }}>{g.attendeeEmail}</div>
                    </div>
                    <div style={{
        ...
        <div>
          <motion.div
            ...
            style={{
              padding: '2rem', borderRadius: '1.5rem', background: 'rgba(var(--glass-rgb),0.03)',
              border: '1px solid rgba(var(--glass-rgb),0.08)', position: 'sticky', top: '2rem'
            }}
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '1rem' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.875rem', color: 'var(--color-text-subtle)', marginTop: '0.5rem' }}>People Inside</div>
              </div>
            </div>
